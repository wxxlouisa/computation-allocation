...
if __name__ == '__main__':
    drop_ratios = np.arange(0.01, 1, 0.01)
    num_drop_ratio = len(drop_ratios)
    decision_offline = DecisionModuleOffline(data_file_name='source.post.csv', train_data_ratio=0.8,
                                             drop_ratios=drop_ratios)

    decision_offline.train()
    np.save('params.npy', dict(zip(drop_ratios, decision_offline.quantiles)))
